...
class Morph extends Hero {
	...
	renderHero() {
		...
		const sphere1 = new Three.Mesh(new Three.SphereBufferGeometry(this.size.w - 1.2), new Three.MeshPhongMaterial({ color: "white" }))
		sphere1.castShadow = true
		sphere1.receiveShadow = true
		group.add(sphere1)
		const body3 = new Three.Mesh(new RoundedBoxGeometry(this.size.w, this.size.h, this.size.d), new Three.MeshNormalMaterial())
		group.add(body3)
		const body4 = new Three.Mesh(new RoundedBoxGeometry(this.size.w, this.size.h, this.size.d), new Three.MeshNormalMaterial())
		group.add(body4)
		const body5 = new Three.Mesh(new RoundedBoxGeometry(this.size.w, this.size.h, this.size.d), new Three.MeshNormalMaterial())
		group.add(body5)


		sphere1.scale.set(.7, .7, .7)
		body3.scale.set(.7, .7, .7)
		body4.scale.set(.7, .7, .7)
		body5.scale.set(.7, .7, .7)

		this.sphere = new Three.Mesh(new Three.SphereBufferGeometry(this.size.w, 20, 20), new Three.MeshNormalMaterial())
		this.sphere.flatShading = true
		this.sphere.material.transparent = true
		this.sphere.material.opacity = .3

		this.pos = JSON.parse(JSON.stringify(this.sphere.geometry.attributes.position.array))
		this.norm = JSON.parse(JSON.stringify(this.sphere.geometry.attributes.normal.array))
	...
	anim(e) {
		this.mesh.children[2].rotation.x = -Math.sin(e) * 1
		this.mesh.children[2].rotation.z = Math.cos(e) * 1
		this.mesh.children[3].rotation.x = Math.cos(e) * 1
		this.mesh.children[3].rotation.z = -Math.sin(e) * 1
		this.mesh.children[4].rotation.x = Math.sin(e) * 1
		this.mesh.children[4].rotation.z = -Math.cos(e) * 1


		var now = Date.now() / 200

		for (var i = 0; i < this.sphere.geometry.attributes.position.count; i++) {
			var ux = this.sphere.geometry.attributes.uv.getX(i) * Math.PI * 16
			var uy = this.sphere.geometry.attributes.uv.getY(i) * Math.PI * 16

			var xangle = (ux + now)
			var xsin = Math.sin(xangle) * .2
			var yangle = (uy + now)
			var ycos = Math.cos(yangle) * .2

			var ix = i * 3
			var iy = i * 3 + 1
			var iz = i * 3 + 2

			this.sphere.geometry.attributes.position.setX(i, this.pos[ix] + this.norm[ix] * (xsin + ycos))
			this.sphere.geometry.attributes.position.setY(i, this.pos[iy] + this.norm[iy] * (xsin + ycos))
			this.sphere.geometry.attributes.position.setZ(i, this.pos[iz] + this.norm[iz] * (xsin + ycos))
	...
	renderBullet() {
		...
		var b = new Three.Mesh(new Three.IcosahedronGeometry(.4), new Three.MeshNormalMaterial())
		b.castShadow = true
		b.receiveShadow = true
		g.add(b)

		var s = new Three.Mesh(new Three.SphereGeometry(.8), new Three.MeshNormalMaterial())
		s.material.transparent = true
		s.material.opacity = .3
		g.add(s)

		g.position.set(this.mesh.position.x, this.mesh.position.y, this.mesh.position.z)
...
class MrSnowMan extends Hero {
	...
	renderHero() {
		...
		const mainBody = new Three.Mesh(new Three.BoxBufferGeometry(this.size.w, this.size.h, this.size.d), new Three.MeshPhongMaterial())
		mainBody.material.transparent = true
		mainBody.material.opacity = 0
		group.add(mainBody)

		const body = new Three.Mesh(new Three.SphereBufferGeometry(this.size.w - 1.5), new Three.MeshPhongMaterial())
		group.add(body)

		const body2 = new Three.Mesh(new Three.SphereBufferGeometry(this.size.w - 2.3), new Three.MeshPhongMaterial())
